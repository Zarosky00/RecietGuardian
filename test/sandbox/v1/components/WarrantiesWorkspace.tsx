...
interface WarrantiesWorkspaceProps {
  isDesktop: boolean;
  theme: any;
  warrantiesList: Receipt[];
...
export const WarrantiesWorkspace: React.FC<WarrantiesWorkspaceProps> = ({
  isDesktop,
  theme,
  warrantiesList,
  setSelectedReceipt,
  ...
  return (
    <Wrapper {...wrapperProps}>
      ...
      <Text style={[styles.tabIntroText, { color: theme.textSecondary, fontFamily: 'Sora, sans-serif', marginTop: 4, marginBottom: 8 }]}>
        Verify product warranty validation ranges.
      </Text>
      <GlassCard 
        intensity="low" 
        bordered={true} 
          ...
          {warrantiesList.length === 0 ? (
            <View style={styles.emptyFeedBox}>
              <Text style={{ color: theme.textSecondary, fontFamily: 'Sora, sans-serif', fontSize: 11 }}>
                No warranty entries found matching selected filters.
              </Text>
            </View>
          ) : (
            warrantiesList.map((r) => (
              <PremiumReceiptCard
                key={r.id}
                receipt={r}
                theme={theme}
